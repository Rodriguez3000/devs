...
@section('contenido')
    <div class="md:flex md:items-center">
        <div class="md:w-1/2 px-10">
            ...
            enctype="multipart/form-data" id="dropzone" class="dropzone border-dashed border-2
            w-full h-96 rounded flex flex-col justify-center items-center">
                @csrf
            </form>
        </div>
        <div class="md:w-1/2 p-10  bg-white  rounded-lg shadow-xl mt-10
        md:mt-0">
            <form action="{{ route('posts.store') }}" method="POST" novalidate>
                ...
                <div class="mb-5">
                    ...
                    name="imagen"
                    type="hidden"
                    {{-- guarda la ultima imagen que se subio --}}
                    value="{{ old('imagen') }}"
                    />
                    @error('imagen')
                        <p class="bg-red-500 text-white my-2 rounded-lg
